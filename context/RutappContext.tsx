...
export function RutappContextProvider({ children }: Props) {
  ...
  const [boolPrincipalMenu, setBoolPrincipalMenu] = useState(false);
  const [textoPrincipal,setTextoPrincipal] = useState(" contacto");
    ...
    <RutappContext.Provider value={{  
      ...
      boolPrincipalMenu, setBoolPrincipalMenu,
      textoPrincipal,setTextoPrincipal
      }}>
      {children}
