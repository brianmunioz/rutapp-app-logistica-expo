  ...
  const crear = async () => {
    ...
    if ((!data.nombre || !data.direccion) && !seleccionContacto) {
      ...
      await db.runAsync('INSERT INTO repartos (nombre,direccion,telefono,tipo_contacto,descripcion,lat,lng,IDContacto,fecha,finalizado) VALUES (?,?,?,?,?,?,?,?,?,?)',
         data.nombre, data.direccion, data.telefono, data.tipo, 
         data.descripcion, lat, lng, 
  ...
  if (seleccionContacto === null) {
    return <View style={{ width: "100%", flex: 1, justifyContent: 'center', height: "100%", alignItems: "center", backgroundColor: "white" }}>
      {contactosArr.length > 0 &&
        <TouchableRipple onPress={() => { setSeleccionContacto(true) }} style={{ borderRadius: 0, flex: 1, width: "100%", justifyContent: "center", alignItems: "center", backgroundColor: "#f76262" }}><Text style={{ fontWeight: 800, fontSize: 20, color: "#000", letterSpacing: 2 }} >Crear reparto con contacto</Text></TouchableRipple>
      ...
      }} style={{ borderRadius: 0, flex: 1, width: "100%", justifyContent: "center", alignItems: "center", backgroundColor: "#99c8db" }}><Text style={{ fontWeight: 800, fontSize: 20, color: "#000", letterSpacing: 2 }}>Crear reparto de forma manual</Text></TouchableRipple>

    </View>
  } else {
    return (
      <ScrollView style={{ width: "100%", paddingHorizontal: 20, paddingTop: 10, flex: 1, backgroundColor: "white", gap: 10 }}>
...

        {
          ...
          <ScrollView style={{ flex: 1 }}>
            ...
            {contactosArr.length > 0 && data.IDContacto &&
              <Card style={{backgroundColor: "#FFFCF0", borderWidth: 1, borderColor: "black"}}>
                <Card.Title title={data.nombre}
                  key={data.IDContacto}
                  subtitle={data.direccion}
                  right={() => <Button mode="elevated" 
                    style={{backgroundColor: "#FFFCF0", marginRight: 10, borderWidth: 1, borderColor: "black"}}
        ...
        <Button mode='elevated' style={{borderColor: "black", borderWidth: 1,marginVertical: 15, backgroundColor: "#FFFCF0"}} onPress={crear}>Crear reparto</Button>
